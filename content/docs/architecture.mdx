...
Toutes les pages Inertia reçoivent automatiquement des props via `HandleInertiaRequests`, qui délègue maintenant la logique à des **services dédiés** :

**Services utilisés** :
- **SharedPropsService** : Orchestration principale des shared props
- **CategoryCacheService** : Gestion du cache des catégories
- **WorkspaceService** : Gestion des équipes utilisateur

**Props disponibles** :
- Configuration globale (`swell.*`)
- Utilisateur authentifié et ses équipes (`auth.user`, `auth.teams`)
- Panier actuel (`cart`)
- Produits populaires pour la recherche (`defaultSearchProducts`)
- Catégories (`categories`)
- Bannières d'information (`infoBanner`) - conditionnel
- Membres du workspace (`workspaceMembers`) - conditionnel sur routes workspace
- Équipes pour invitations (`invitableTeams`) - conditionnel sur routes workspace

<Callout type="info">
  **Note :** Le projet utilise [Laravel Wayfinder](https://github.com/laravel/wayfinder) pour la génération de routes côté frontend, remplaçant l'ancienne dépendance Ziggy.
</Callout>

**Avantages** :
- ✅ Données essentielles disponibles partout sans duplication
- ✅ Séparation des responsabilités via services
- ✅ Cache stratifié (indéfini pour données statiques, 30-60s pour données dynamiques)
- ✅ Services testables et réutilisables

Voir [HandleInertiaRequests](/handle-inertia-requests) pour plus de détails.
